...
Эндпоинт `/api/miniapp/cases` возвращает публичные данные витрины (id, название, цену в звёздах, миниатюру и короткое описание) из `src/main/resources/config/cases.yaml`. Ответы помечаются `Cache-Control: no-store`, чтобы Mini App всегда подтягивал актуальный конфиг.
